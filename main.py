...
def get_player_info(player_id):
    url = f"https://kallu-add-friend.vercel.app/adding_friend?uid=4357924270&password=M_TZ4KQ_BY_SPIDEERIO_GAMING_2CMRU&friend_uid={player_id}"
...
def get_player_bio(uid):
    try:
        url = f"https://kallu-info-api.vercel.app/accinfo?uid={uid}"
        res = requests.get(url)
        if res.status_code == 200:
            data = res.json()
...
def get_random_color():
    colors = [
        "[FF0000]", "[00FF00]", "[0000FF]", "[FFFF00]", "[3CB371]", "[FFC107]", "[FFFFFF]", "[FFA500]",
        "[A52A2A]", "[800080]", "[000000]", "[808080]", "[C0C0C0]", "[FFC0CB]", "[FFD700]", "[ADD8E6]",
        "[90EE90]", "[D2691E]", "[DC143C]", "[FFC107]", "[9400D3]", "[F08080]", "[20B2AA]", "[FFC107]",
        "[FFC107]", "[B22222]", "[3CB371]", "[DAA520]", "[00BFFF]", "[00FF7F]", "[4682B4]", "[6495ED]",
        "[5F9EA0]", "[DDA0DD]", "[E6E6FA]", "[B0C4DE]", "[556B2F]", "[8FBC8F]", "[2E8B57]", "[3CB371]",
        "[6B8E23]", "[808000]", "[B8860B]", "[CD5C5C]", "[8B0000]", "[FFC107]", "[FF8C00]", "[BDB76B]",
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/emote'):
                            print('Processing Adding command in any chat type')
                            
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[C][B]⚡[FFC107]Send Single Emote\n⚡[3CB371]/e [uid] [id]\n⚡[FFC107]Fast Emote (25x\n⚡[3CB371]/fast [uid] [id]⚡ [FFC107]Custom Emote (X times)\n⚡[3CB371]/p [uid] [id] [x]\n⚡[FFC107]Send Evolution Emote\n⚡[3CB371]/evo [uid] [1-21]\n⚡[FFC107]Fast Evo (25x)\n⚡ [3CB371]/evo_fast [uid] [1-21]\n⚡[FFC107]Custom Evo (X times)\n⚡[3CB371]/evo_c [uid] [1-21] [x]\n⚡[FFC107]Auto Cycle All Evo Emotes\n⚡[3CB371]/random [uid]\n⚡[FFC107]Stop Evo Emote Cycle\n⚡[3CB371]/sevos\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_uid = parts[1]
                                initial_message = f"[\n"
        ...
        # Use ThreadPoolExecutor to avoid blocking the async loop
                        ...
                        if inPuTMsG.strip() == "/level":
                            # Process /admin command in any chat type
                            admin_message = """
[C][B][3CB371]═══⚡ AUTO START BOT ⚡═══[FFC107][B]
 ⚡[FFC107]Start level up bot (24x7)
   ⚡[3CB371]/lw [team_code]
 ⚡[FFC107]Stop level up Bot
    ⚡[3CB371]/stop level
[FFC107]━━━━━━━━━━━━[3CB371]
"""
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/fix'):
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 2:
                                ...
                                try:
                                    ...
                                    # Create spam packet
                                    spam_packet = await Room_Spam(target_uid, room_id, "NoTmeowL", key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/spam'):
                            ...
                            if len(parts) < 2:
                                ...
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                                                                                             ...
                                                                                             #JOIN ROOM       
                        if inPuTMsG.strip().startswith('/nm'):
                            ...
                            if len(parts) < 3:
                                ...
                                initial_msg = f"/createroom BOTROOM 0000 4\n"
                                await safe_send_message(response.Data.chat_type, initial_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG.strip().startswith('/gst'):
                        ...
                        if inPuTMsG.strip().startswith('/lw'):
                            print('Processing /level auto-start command')
                            global auto_start_running, auto_start_teamcode, stop_auto, auto_start_task
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /level (team_code)\nExample: /level 123456\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                team_code = parts[1]
                                ...
                                # Send initial message
                                initial_msg = f"""
[B][C][FFC107]🤖 AUTO START BOT ACTIVATED!
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/sinv'):
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            #  Group Commands 
                            group_commands = """[C][B][3CB371]═══⚡NM BOT V1⚡═══[FFC107][B]
[FFFF00][1]Create 3 Player Group[FFFFFF]-/3
[FFFF00][2]Create 5 Player Group[FFFFFF]-/5
[FFFF00][3]Create 6 Player Group[FFFFFF]-/6
[FFFF00][4]Invite Bot In Grup[FFFFFF]-/x [t-c]
[FFFF00][5]Invite Any Player [FFFFFF]-/inv [uid]
[FFFF00][6]Make Bot leave [FFFFFF]-/solo
[FFFF00][6]Room Spam [FFFFFF]-/room [uid]
[FFFF00][6] Invite Custom Badge[FFFFFF]-/s1,s2,s3,s4,s5
[FFFF00][7]Start Match [FFFFFF]-/start
[FFFF00][8]Open Emote Cmds [FFFFFF]-/emote
[FFFF00][9]Open Levelup Cmds [FFFFFF]-/level
[FFC107]━━━━━━━━━━━━[3CB371]"""
                            await safe_send_message(response.Data.chat_type, group_commands, uid, chat_id, key, iv)
                            await asyncio.sleep(0.2)

                            #  Advanced Commands 
                            advanced_commands = """[C][B][3CB371]═══⚡ Subscribe To NoTmeowL⚡═══[FFC107][B]
[b][00FFFF][1]Get Player bio [FFFFFF]-/bio [uid]
[00FFFF][2]Get Player Info [FFFFFF]-/info [uid]
[00FFFF][3]Talk With Ai [FFFFFF]-/ai [?]
[00FFFF][4]check Bot Status[FFFFFF]-/status
[00FFFF][5]Spm Atk In Chat[FFFFFF]-/atk [msg]
[00FFFF][6]Invite Multiple badge [FFFFFF]/biv [uid]
[00FFFF][7]LaG Player,Team[FFFFFF]-/lag [tcode]
[00FFFF][8]Spam Invite [FFFFFF]-/sinv [uid]
[00FFFF][9]Join Ghost Pct [FFFFFF]-/gst [tcode]
[FFC107]━━━━━━━━━━━━[3CB371]"""
...
async def MaiiiinE():
    Uid , Pw = '4342156733','E64567D970CCBCDFCCD68427F0895A1860D671A0495BA8854C189492CAAC66A7'
    ...
    os.system('clear')
    print("Initializing NoTmeowL Bot...")
    print("")
    print("Loading :")
    print("")
    time.sleep(0.5)
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("👋 Thanks for using NoTmeowL")
    sys.exit(0)
